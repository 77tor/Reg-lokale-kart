...
// --- SIDE 4: UTVIKLING OVER TID (Optimalisert med Linjediagram) ---
...
try {
    ...
    // Henter data (samme logikk som før)
        ...
        for (const pKey of Object.keys(fData)) {
            ...
            Object.keys(trinnData).forEach(kNavn => {
                ...
                const kElever = Object.keys(kData).filter(n => {
                    ...
                    return e && kData[n].oppgaver && !kData[n].slettet && 
                           historiskStartAar >= parseInt(e.startAar) && 
                           (!e.sluttAar || historiskStartAar <= parseInt(e.sluttAar));
                });
                ...
                kElever.forEach(n => {
                    ...
                    if (kNavn === klasse) {
                        ...
                        if (eSum <= aOppsett.grenseTotal) kKritiske++;
                        else if ((eSum / aMaks) * 100 < 70) kLav++;
                    }
                });
            });
            ...
            if (klasseAntall > 0) {
                historikkRader.push({ 
                    visning: `${pKey.slice(0,1)}. ${aKey.slice(2,4)}/${aKey.slice(7,9)}`, // Eks: H. 24/25
                    klasseProsent: ((klasseSum / klasseAntall) / aMaks) * 100,
                    proveProsent: ((totalProveSum / totalProveAntall) / aMaks) * 100,
                    kritiske: kKritiske, lavMestring: kLav,
    ...
    if (historikkRader.length > 0) {
        // --- SVG LINJEDIAGRAM ---
        const w = 800; // Bredde på diagrammet
        const h = 120; // Høyde på diagrammet (lavt og kompakt)
        const padding = 30;
        const step = (w - (padding * 2)) / (Math.max(historikkRader.length - 1, 1));
        ...
        historikkRader.forEach((r, i) => {
            ...
            const yKlasse = h - (r.klasseProsent * (h / 100));
            const yProve = h - (r.proveProsent * (h / 100));
            
            pointsKlasse += `${x},${yKlasse} `;
            pointsProve += `${x},${yProve} `;
            
            // Punkt-markører og tekst
            labels += `
                <circle cx="${x}" cy="${yKlasse}" r="3" fill="#3498db" />
                <text x="${x}" y="${h + 15}" font-size="9" text-anchor="middle" fill="#666" transform="rotate(-20, ${x}, ${h+15})">${r.visning}</text>
                <text x="${x}" y="${yKlasse - 8}" font-size="8" font-weight="bold" text-anchor="middle" fill="#2c3e50">${r.klasseProsent.toFixed(0)}%</text>
            `;
        });

        htmlSide4 += `
            <div style="width:100%; text-align:center; margin-bottom:40px; background:#fff; padding:10px 0;">
                <svg width="${w}" height="${h + 40}" viewBox="0 0 ${w} ${h + 40}">
                    <line x1="${padding}" y1="${h}" x2="${w-padding}" y2="${h}" stroke="#eee" stroke-width="1" />
                    <line x1="${padding}" y1="0" x2="${w-padding}" y2="0" stroke="#eee" stroke-width="1" />
                    <text x="${padding-5}" y="${h}" font-size="8" text-anchor="end">0%</text>
                    <text x="${padding-5}" y="8" font-size="8" text-anchor="end">100%</text>

                    <polyline points="${pointsProve}" fill="none" stroke="#ccc" stroke-width="2" stroke-dasharray="4" />
                    <polyline points="${pointsKlasse}" fill="none" stroke="#3498db" stroke-width="3" stroke-linejoin="round" />
                    
                    ${labels}
                    
                    <g transform="translate(${w-120}, -10)">
                        <line x1="0" y1="0" x2="15" y2="0" stroke="#3498db" stroke-width="3" />
                        <text x="20" y="4" font-size="9">Klassen</text>
                        <line x1="0" y1="12" x2="15" y2="12" stroke="#ccc" stroke-width="2" stroke-dasharray="3" />
                        <text x="20" y="16" font-size="9">Trinn-snitt</text>
                    </g>
                </svg>
            </div>`;
            ...
            <table style="width:100%; border-collapse:collapse; font-size:12px;">
                <thead>
                    <tr style="background:#f8f9fa;">
                        <th style="padding:6px; border-bottom:2px solid #dee2e6;">Periode</th>
                        <th style="padding:6px; border-bottom:2px solid #dee2e6;">Klasse %</th>
                        <th style="padding:6px; border-bottom:2px solid #dee2e6;">Trinn %</th>
                        <th style="padding:6px; border-bottom:2px solid #dee2e6;">Diff.</th>
                        <th style="padding:6px; border-bottom:2px solid #dee2e6;">Lav</th>
                        <th style="padding:6px; border-bottom:2px solid #dee2e6;">Kritisk</th>
                    </tr>
                </thead>
                <tbody>`;

        historikkRader.forEach(r => {
            const erAktiv = r.sort.includes(aar) && r.sort.includes(periode === "Høst" ? "1" : "2");
            const aktivStil = erAktiv ? 'background:#e8f4fd; font-weight:bold;' : '';
            const diff = r.klasseProsent - r.proveProsent;
            
            htmlSide4 += `
                <tr style="${aktivStil}">
                    <td style="padding:5px; border-bottom:1px solid #eee;">${r.visning}</td>
                    <td style="padding:5px; border-bottom:1px solid #eee;">${r.klasseProsent.toFixed(1)}%</td>
                    <td style="padding:5px; border-bottom:1px solid #eee; color:#666;">${r.proveProsent.toFixed(1)}%</td>
                    <td style="padding:5px; border-bottom:1px solid #eee; color:${diff >= 0 ? 'green':'red'};">${diff >= 0 ? '+':''}${diff.toFixed(1)}%</td>
                    <td style="padding:5px; border-bottom:1px solid #eee;">${r.lavMestring}</td>
                    <td style="padding:5px; border-bottom:1px solid #eee; ${r.kritiske > 0 ? 'color:red;' : ''}">${r.kritiske}</td>
                </tr>`;
        });
        htmlSide4 += `</tbody></table>`;

        // Oppsummerings-tekst (Beholdt fra din opprinnelige logikk)
        ...
        if (historikkRader.length > 1) {
            ...
            if (endring > 3) { utviklingTekst = `<b>Fremgang:</b> +${endring.toFixed(1)}% siden sist.`; utviklingFarge = "#27ae60"; }
            else if (endring < -3) { utviklingTekst = `<b>Nedgang:</b> ${endring.toFixed(1)}% siden sist.`; utviklingFarge = "#e67e22"; }
            else { utviklingTekst = `Stabil utvikling.`; }
        }

        htmlSide4 += `
            <div style="margin-top:15px; display: flex; gap: 10px;">
                <div style="flex:1; padding:10px; border-left:4px solid ${utviklingFarge}; background:#f9f9f9; font-size:12px;">
                    <strong>Utvikling:</strong> ${utviklingTekst}
                </div>
                <div style="flex:1; padding:10px; border-left:4px solid #2c3e50; background:#f9f9f9; font-size:12px;">
                    <strong>Status:</strong> ${siste.klasseProsent > siste.proveProsent ? 'Over' : 'Under'} snittet for trinnet.
                </div>
            </div>`;
    }
} catch(err) {

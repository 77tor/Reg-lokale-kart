...
function tegnTabell() {
    ...
    const overskrift = `Kartlegging i ${f} - ${t}${k} - ${p} ${a}`;
    if (document.getElementById('dynamiskOverskrift')) document.getElementById('dynamiskOverskrift').innerText = overskrift;
    if (document.getElementById('printTittel')) document.getElementById('printTittel').innerText = overskrift;

    if (!oppsett) { tBody.innerHTML = "<tr><td colspan='100%'>Ingen mal funnet.</td></tr>"; return; }
    ...
    Object.keys(elevRegister).sort().forEach(navn => {
        ...
        if (cTrinn === vTrinn && e.startKlasse === k) {
            if (!(lagredeResultater[navn] && lagredeResultater[navn].skjult)) aktive.push(navn);
        }
    });
    ...
    aktive.forEach(navn => {
        ...
        if (d && d.oppgaver) {
            oppsett.oppgaver.forEach((o, i) => {
                ...
                let cls = (f === "Lesing" && o.grense !== -1 && poeng <= o.grense) ? 'class="alert-low"' : '';
                rad += `<td ${cls}>${poeng}</td>`;
            });

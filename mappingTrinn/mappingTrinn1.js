window.mappingTrinn1 = {
  "Høst": {
    "1": { 
      navn: "Hvor mange av hver? (Kategorisering)", 
      tema: "Sortering", 
      bøker: [
        { bok: "grunnbok1A", side: 6 },
        { bok: "Øvebok1", side: 4 }
      ]
    },
    "2": { 
      navn: "Hvor mye? (Penger)", 
      tema: "Myntene våre", 
      bøker: [
        { bok: "grunnbok1B", side: 76 }, // Introduseres ofte senere, men relevant tema
        { bok: "Øvebok1", side: 116 }
      ]
    },
    "3": { 
      navn: "Like mange (Tellestreker)", 
      tema: "Like mange. Flere. Færre", 
      bøker: [
        { bok: "grunnbok1A", side: 10 },
        { bok: "Øvebok1", side: 8 }
      ]
    },
    "4": { 
      navn: "Tallrekker", 
      tema: "Tall på rekke og rad", 
      bøker: [
        { bok: "grunnbok1A", side: 24 },
        { bok: "Øvebok1", side: 18 }
      ]
    },
    "5": { 
      navn: "Skriv tallene i rekkefølge", 
      tema: "Tall til 10 på rekke og rad", 
      bøker: [
        { bok: "grunnbok1A", side: 46 },
        { bok: "Øvebok1", side: 32 }
      ]
    },
    "6": { 
      navn: "Mengdeforståelse (Tre av hver)", 
      tema: "Tallene 0-6", 
      bøker: [
        { bok: "grunnbok1A", side: 16 },
        { bok: "Øvebok1", side: 12 }
      ]
    },
    "7": { 
      navn: "Fargelegg antall", 
      tema: "Tallene 7 og 8 / 9 og 10", 
      bøker: [
        { bok: "grunnbok1A", side: 30 },
        { bok: "Øvebok1", side: 24 }
      ]
    },
    "8": { 
      navn: "Regn ut (Enkel addisjon)", 
      tema: "Hva er addisjon?", 
      bøker: [
        { bok: "grunnbok1A", side: 72 },
        { bok: "Øvebok1", side: 52 }
      ]
    }
  },
  "Vår": {
    "1": { 
      navn: "Hvor mye? (Penger og terninger)", 
      tema: "Myntene våre", 
      bøker: [
        { bok: "grunnbok1B", side: 76 },
        { bok: "Øvebok1", side: 116 }
      ]
    },
    "2": { 
      navn: "Skriv tallene som mangler", 
      tema: "Tallsymbolene til 20", 
      bøker: [
        { bok: "grunnbok1B", side: 58 },
        { bok: "Øvebok1", side: 104 }
      ]
    },
    "3": { 
      navn: "Skriv riktig tall på kortet", 
      tema: "Tiere og enere", 
      bøker: [
        { bok: "grunnbok1B", side: 70 },
        { bok: "Øvebok1", side: 112 }
      ]
    },
    "4": { 
      navn: "Trekk strek til tallinja", 
      tema: "Tallenes rekkefølge", 
      bøker: [
        { bok: "grunnbok1B", side: 64 },
        { bok: "Øvebok1", side: 108 }
      ]
    },
    "5": { 
      navn: "Tallrekker (Rutenett)", 
      tema: "Tallenes rekkefølge", 
      bøker: [
        { bok: "grunnbok1B", side: 64 },
        { bok: "Øvebok1", side: 108 }
      ]
    },
    "6": { 
      navn: "Sortering (Stigende rekkefølge)", 
      tema: "Sammenlikne tall", 
      bøker: [
        { bok: "grunnbok1B", side: 82 },
        { bok: "Øvebok1", side: 120 }
      ]
    },
    "7": { 
      navn: "Regn ut (Addisjon)", 
      tema: "Addisjon til 20", 
      bøker: [
        { bok: "grunnbok1B", side: 94 },
        { bok: "Øvebok1", side: 128 }
      ]
    },
    "8": { 
      navn: "Regn ut (Subtraksjon)", 
      tema: "Subtraksjon til 20", 
      bøker: [
        { bok: "grunnbok1B", side: 100 },
        { bok: "Øvebok1", side: 132 }
      ]
    },
    "9": { 
      navn: "Finn tallet som mangler", 
      tema: "Det ukjente tallet", 
      bøker: [
        { bok: "grunnbok1B", side: 112 },
        { bok: "Øvebok1", side: 140 }
      ]
    },
    "10": { 
      navn: "Hvor mye til sammen? (Penger)", 
      tema: "Myntene våre", 
      bøker: [
        { bok: "grunnbok1B", side: 76 },
        { bok: "Øvebok1", side: 116 }
      ]
    }
  }
};

// Nederst i mappingTrinn1.js
window.mappingTrinn1 = mappingTrinn1;